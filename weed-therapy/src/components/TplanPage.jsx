  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      ...
      <div className="absolute flex flex-col items-center justify-center text-center">
        <span className="text-white font-bold text-2xl leading-none">{percent}%</span>
...
const StatCard = ({ icon, label, value, color = 'text-white' }) => (
  <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-2xl p-4 flex flex-col gap-1 backdrop-blur-sm">
...
// ─── Feed Items (static — will be AI-generated in Phase 2) ───────────────────
const feedItems = [
  { type: 'article', emoji: '🧠', tag: 'Science', title: 'Why your brain craves weed after stress',
    desc: 'The endocannabinoid system plays a key role in how we respond to stress. Understanding it helps you break the cycle.',
    time: '3 min read', color: 'from-blue-900/40 to-zinc-900/40', border: 'border-blue-700/30' },
  { type: 'movie', emoji: '🎬', tag: 'Watch Tonight', title: 'The Social Dilemma',
    desc: 'A gripping documentary that will keep your mind engaged and away from cravings for 90 minutes.',
    time: 'Netflix · 89 min', color: 'from-purple-900/40 to-zinc-900/40', border: 'border-purple-700/30' },
  { type: 'tip', emoji: '💡', tag: 'Coping Tip', title: 'The 5-4-3-2-1 grounding technique',
    desc: 'When a craving hits: name 5 things you see, 4 you can touch, 3 you hear, 2 you smell, 1 you taste.',
    time: '1 min read', color: 'from-green-900/40 to-zinc-900/40', border: 'border-green-700/30' },
  { type: 'movie', emoji: '🎮', tag: 'Distraction', title: 'Try a 20-min gaming session',
    desc: 'Short gaming sessions activate dopamine pathways and can effectively replace craving urges.',
    time: 'Activity · 20 min', color: 'from-yellow-900/40 to-zinc-900/40', border: 'border-yellow-700/30' },
  { type: 'article', emoji: '💰', tag: 'Motivation', title: 'How much money are you actually saving?',
    desc: "Users who reduce by 50% save an average of £180/month. That's a weekend trip every 2 months.",
    time: '2 min read', color: 'from-emerald-900/40 to-zinc-900/40', border: 'border-emerald-700/30' },
];

const FeedCard = ({ item }) => (
  <div className={`bg-gradient-to-br ${item.color} border ${item.border} rounded-2xl p-4 flex gap-3 shrink-0`} style={{ width: 260 }}>
...
// ─── Reminder Toggles (local state — Phase 3 will wire to push API) ──────────
        ...
        {reminders.map(r => (
          <div key={r.id} className="flex items-center justify-between bg-zinc-800/60 rounded-xl px-4 py-3">
            ...
            <button onClick={() => toggle(r.id)}
              className={`w-10 h-5 rounded-full transition-colors relative ${r.active ? 'bg-green-600' : 'bg-zinc-700'}`}>
...
const QuickLogModal = ({ plan, onClose, onLogged }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-lg bg-zinc-900 border border-zinc-700 rounded-t-3xl p-6 pb-8 space-y-5"
        onClick={e => e.stopPropagation()} style={{ animation: 'slideUp 0.3s ease' }}>
        {success ? (
          ...
          <>
              ...
              <div className="flex gap-2">
                {moods.map(m => (
                  <button key={m.val} onClick={() => setMood(m.val)}
                    className={`flex-1 flex flex-col items-center py-2 rounded-xl border transition text-xs ${
                      mood === m.val ? 'bg-green-600/30 border-green-500 text-green-300' : 'bg-zinc-800 border-zinc-700 text-zinc-400'
                    }`}>
            ...
            <div>
              ...
              <div className="flex gap-3 items-center">
                <button onClick={() => setUsage(v => String(Math.max(0, Number(v) - 0.5)))}
                  className="w-10 h-10 rounded-xl bg-zinc-800 text-white text-xl font-bold flex items-center justify-center">−</button>
                <input type="number" value={usage} onChange={e => setUsage(e.target.value)}
                  placeholder="0" min="0" step="0.5"
                  className="flex-1 bg-zinc-800 text-white text-center text-2xl font-bold rounded-xl py-3 outline-none focus:ring-2 focus:ring-green-500" />
                <button onClick={() => setUsage(v => String(Number(v) + 0.5))}
                  className="w-10 h-10 rounded-xl bg-zinc-800 text-white text-xl font-bold flex items-center justify-center">+</button>
              </div>
            </div>
            ...
            <input type="text" value={notes} onChange={e => setNotes(e.target.value)}
              ...
              className="w-full bg-zinc-800 text-white rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-green-500" />

            <button onClick={handleLog} disabled={!usage || logging}
              className="w-full bg-green-600 hover:bg-green-500 disabled:opacity-40 text-white font-bold py-3.5 rounded-xl transition text-base">
              {logging ? 'Saving...' : 'Log Usage ✓'}
            </button>
          </>
        )}
      </div>
  ...
  if (preview) return (
    <div className="space-y-5 animate-fadeIn">
      <div className="bg-gradient-to-br from-green-900/50 to-emerald-900/30 border border-green-700/50 rounded-3xl p-6">
        <div className="flex items-center gap-2 mb-3"><span className="text-2xl">✨</span><span className="text-green-300 font-semibold text-sm">Your AI Therapist</span></div>
      ...
      <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-3xl p-5">
        ...
        <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
          {preview.weeklyTargets.map((wk) => (
            <div key={wk.week} className="flex items-center gap-3 bg-zinc-800/60 rounded-xl p-3">
      ...
      <button onClick={() => onPlanCreated(preview)}
        className="w-full bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-500 hover:to-emerald-400 text-white font-bold py-4 rounded-2xl transition text-base shadow-lg shadow-green-900/30">
        Start My Journey 🌿
      </button>
    </div>
  );
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center justify-center gap-2">
        {steps.map((_, i) => (
          <div key={i} className={`rounded-full transition-all duration-300 ${i === step ? 'w-6 h-2 bg-green-500' : i < step ? 'w-2 h-2 bg-green-700' : 'w-2 h-2 bg-zinc-700'}`} />
        ))}
      </div>
      ...
      {error && <div className="bg-red-900/30 border border-red-700/50 text-red-300 text-sm rounded-xl p-3 text-center">{error}</div>}
      ...
      {step === 0 && (
        <div className="space-y-4">
          <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-2xl p-5 space-y-4">
            <div>
              ...
              <div className="flex gap-2">
                <input type="number" value={form.currentUsage} onChange={e => setForm({ ...form, currentUsage: e.target.value })}
                  ...
                  className="flex-1 bg-zinc-800 text-white rounded-xl px-4 py-3 text-lg font-bold outline-none focus:ring-2 focus:ring-green-500 border border-zinc-700" />
                <select value={form.unit} onChange={e => setForm({ ...form, unit: e.target.value })}
                  className="bg-zinc-800 text-white rounded-xl px-3 py-3 text-sm outline-none border border-zinc-700">
            ...
            <div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {['4', '8', '12'].map(w => (
                  <button key={w} onClick={() => setForm({ ...form, durationWeeks: w })}
                    className={`py-2.5 rounded-xl text-sm font-semibold border transition ${form.durationWeeks === w ? 'bg-green-600 border-green-500 text-white' : 'bg-zinc-800 border-zinc-700 text-zinc-400'}`}>
                    {w} weeks
                  </button>
                ))}
              </div>
          ...
          <button onClick={() => { if (!form.currentUsage) return setError('Enter your current usage.'); setError(''); setStep(1); }}
            className="w-full bg-green-600 hover:bg-green-500 text-white font-bold py-3.5 rounded-2xl transition">Continue →</button>
        </div>
      )}
      ...
      {step === 1 && (
        <div className="space-y-4">
          <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-2xl p-5">
            ...
            <div className="flex gap-3 items-center">
              <button onClick={() => setForm(f => ({ ...f, targetUsage: String(Math.max(0, Number(f.targetUsage) - 0.5)) }))}
                className="w-12 h-12 rounded-xl bg-zinc-800 text-white text-2xl font-bold flex items-center justify-center border border-zinc-700">−</button>
              <input type="number" value={form.targetUsage} onChange={e => setForm({ ...form, targetUsage: e.target.value })}
                min="0" step="0.5"
                className="flex-1 bg-zinc-800 text-white text-center text-3xl font-bold rounded-xl py-3 outline-none focus:ring-2 focus:ring-green-500 border border-zinc-700" />
              <button onClick={() => setForm(f => ({ ...f, targetUsage: String(Number(f.targetUsage) + 0.5) }))}
                className="w-12 h-12 rounded-xl bg-zinc-800 text-white text-2xl font-bold flex items-center justify-center border border-zinc-700">+</button>
            </div>
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(0)} className="flex-1 bg-zinc-800 border border-zinc-700 text-white font-semibold py-3.5 rounded-2xl transition">← Back</button>
            <button onClick={() => { setError(''); setStep(2); }} className="flex-1 bg-green-600 hover:bg-green-500 text-white font-bold py-3.5 rounded-2xl transition">Continue →</button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-4">
          <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-2xl p-5">
            ...
            <textarea value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })}
              placeholder="e.g. Better sleep, save money, mental clarity, be more present..."
              rows={4} className="w-full bg-zinc-800 text-white rounded-xl px-4 py-3 text-sm outline-none focus:ring-2 focus:ring-green-500 border border-zinc-700 resize-none" />
            <div className="flex flex-wrap gap-2 mt-3">
              {['Better sleep 😴', 'Save money 💰', 'Mental clarity 🧠', 'More energy ⚡', 'Be present 🧘'].map(tag => (
                <button key={tag} onClick={() => setForm(f => ({ ...f, reason: f.reason ? f.reason + ', ' + tag : tag }))}
                  className="text-xs px-3 py-1.5 rounded-full bg-zinc-800 border border-zinc-700 text-zinc-400 hover:border-green-600 hover:text-green-400 transition">
                  {tag}
                </button>
              ))}
            </div>
          ...
          <div className="flex gap-3">
            <button onClick={() => setStep(1)} className="flex-1 bg-zinc-800 border border-zinc-700 text-white font-semibold py-3.5 rounded-2xl transition">← Back</button>
            <button onClick={handleGenerate} disabled={loading}
              className="flex-1 bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-500 hover:to-emerald-400 disabled:opacity-50 text-white font-bold py-3.5 rounded-2xl transition">
              {loading ? (
...
const ProgressView = ({ plan, onRefresh }) => {
  ...
  const daysSinceStart = Math.floor((new Date() - new Date(plan.createdAt || Date.now())) / (1000 * 60 * 60 * 24));
  ...
  const avgThisWeek = thisWeekLogs.length
    ? (thisWeekLogs.reduce((s, l) => s + l.actualUsage, 0) / thisWeekLogs.length).toFixed(1)
    : null;

  const totalLogged = plan.progressLogs?.reduce((s, l) => s + l.actualUsage, 0) || 0;
  const expectedWithoutPlan = plan.goal.currentUsage * daysSinceStart;
  const saved = Math.max(0, expectedWithoutPlan - totalLogged);
  const moneySaved = (saved * 10).toFixed(0);

  // Use server-computed streak (falls back to 0 if not present)
    ...
    <div className="space-y-5">
      ...
      <div className="flex bg-zinc-900/80 border border-zinc-700/50 rounded-2xl p-1 gap-1">
        {tabs.map(t => (
          <button key={t.id} onClick={() => setActiveTab(t.id)}
            className={`flex-1 flex flex-col items-center py-2 rounded-xl text-xs font-semibold transition ${
              activeTab === t.id ? 'bg-green-600 text-white' : 'text-zinc-500 hover:text-zinc-300'
            }`}>
        ...
        <div className="space-y-4 animate-fadeIn">
          ...
          <div className="bg-gradient-to-br from-zinc-900 to-zinc-800 border border-zinc-700/50 rounded-3xl p-6">
            ...
            <button onClick={() => setShowLogModal(true)}
              className={`w-full mt-5 text-white font-bold py-3.5 rounded-2xl transition shadow-lg flex items-center justify-center gap-2 ${
                plan.todayLogged
                  ? 'bg-zinc-700 hover:bg-zinc-600 border border-zinc-600 shadow-zinc-900/30'
                  : 'bg-gradient-to-r from-green-600 to-emerald-500 hover:from-green-500 hover:to-emerald-400 shadow-green-900/30'
              }`}>
          ...
          <div className="grid grid-cols-3 gap-3">
            <StatCard icon="📅" label="Days logged" value={plan.progressLogs?.length ?? 0} color="text-blue-400" />
            <StatCard icon="💰" label="Est. saved" value={`£${moneySaved}`} color="text-yellow-400" />
            <StatCard icon="📉" label="Reduction"
              value={`${Math.round(((plan.goal.currentUsage - (Number(avgThisWeek) || plan.goal.currentUsage)) / plan.goal.currentUsage) * 100)}%`}
          ...
          <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-2xl p-4">
            ...
            <div className="w-full bg-zinc-800 rounded-full h-2.5 mb-2">
              <div className="bg-gradient-to-r from-green-600 to-emerald-400 h-2.5 rounded-full transition-all duration-700"
      ...
      {activeTab === 'schedule' && (
        <div className="space-y-3 animate-fadeIn">
          ...
          {plan.weeklyTargets.map((wk) => {
            const isCurrent = wk.week === plan.currentWeek;
            const isPast = wk.week < plan.currentWeek;
            return (
              <div key={wk.week} className={`flex items-center gap-4 rounded-2xl p-4 border transition ${
      ...
      {activeTab === 'feed' && (
        <div className="space-y-4 animate-fadeIn">
          ...
          <div ref={feedRef} className="flex gap-3 overflow-x-auto pb-2" style={{ scrollbarWidth: 'none' }}>
            {feedItems.map((item, i) => <FeedCard key={i} item={item} />)}
          </div>
          <ReminderToggles />
          <div className="bg-zinc-900/80 border border-zinc-700/50 rounded-2xl p-4">
            <p className="text-white font-semibold text-sm mb-3">🎬 Watch Instead Tonight</p>
            <div className="space-y-2">
              {[
                { title: 'The Social Dilemma', platform: 'Netflix', genre: 'Documentary', emoji: '🧠' },
                { title: 'Interstellar',        platform: 'Prime',   genre: 'Sci-Fi',      emoji: '🚀' },
                { title: "Chef's Table",        platform: 'Netflix', genre: 'Food Doc',    emoji: '🍽️' },
              ].map((m, i) => (
                <div key={i} className="flex items-center gap-3 bg-zinc-800/60 rounded-xl px-4 py-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-zinc-500 text-xs">{m.platform} · {m.genre}</p>
                  </div>
      ...
      {activeTab === 'badges' && (
        <div className="space-y-4 animate-fadeIn">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {earnedBadges.filter(b => b.earned).map(b => (
                <div key={b.id} className="bg-gradient-to-br from-green-900/40 to-zinc-900/60 border border-green-700/40 rounded-2xl p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {earnedBadges.filter(b => !b.earned).map(b => (
                <div key={b.id} className="bg-zinc-900/60 border border-zinc-800 rounded-2xl p-4 flex items-center gap-3 opacity-50">
          ...
          <div className="bg-gradient-to-br from-green-900/40 to-emerald-900/30 border border-green-700/40 rounded-2xl p-5 text-center">
            <p className="text-white font-bold text-lg">Week {plan.currentWeek} Complete 🌿</p>
            <p className="text-zinc-400 text-sm mt-1">Share your progress with friends</p>
            <button className="mt-3 bg-green-600 hover:bg-green-500 text-white text-sm font-semibold px-6 py-2.5 rounded-xl transition">
              Share Progress Card
            </button>
          </div>
        </div>
...
const TplanPage = () => {
  ...
  const fetchPlan = async () => {
    ...
    setFetchError('');
    try {
      const { data } = await axios.get(`${API}/api/tplan/active`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setPlan(data.plan || null);
    } catch (err) {
      if (err.response?.status === 404) {
        setPlan(null); // No plan yet — show setup
      } else {
        setFetchError('Failed to load your plan. Please refresh.');
      }
    } finally {
      setLoading(false);
    }
      ...
      <div className="max-w-lg mx-auto px-4 py-6 pb-20">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button onClick={fetchPlan}
            className="w-9 h-9 rounded-xl bg-zinc-800 border border-zinc-700 flex items-center justify-center text-zinc-400 hover:text-white transition">↻</button>
        </div>
        ...
        {loading ? (
          ...
          <div className="bg-red-900/30 border border-red-700/50 text-red-300 text-sm rounded-2xl p-4 text-center">{fetchError}</div>
        ) : plan ? (
